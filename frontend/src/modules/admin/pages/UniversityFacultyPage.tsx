...
export const UniversityFacultyPage: React.FC = () => {
  ...
  const handleCreateFaculty = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.error?.message || 'Failed to create faculty account.');
    } finally {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <h1 className="text-2xl font-black text-slate-900 dark:text-white">Institutional Faculty Management</h1>
      ...
      {isLoading ? (
        ...
        <Card className="text-center py-12">
          ...
          <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">No faculty members onboarded yet</h3>
          <p className="text-sm text-slate-500 mb-4">Add your university professors to guide student challenge teams.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {facultyList.map((fac) => (
            <Card key={fac.id} className="border-slate-200 dark:border-slate-800 flex flex-col justify-between">
              <div>
                <div className="flex items-start justify-between gap-2 mb-3">
                  <div>
                    <h4 className="text-base font-bold text-slate-900 dark:text-slate-100">{fac.full_name}</h4>
                    <span className="text-xs text-indigo-600 dark:text-indigo-400 font-semibold">{fac.designation}</span>
                  </div>
                ...
                {fac.research_areas && fac.research_areas.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {fac.research_areas.map((r, idx) => (
                      <span key={idx} className="text-[10px] bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-slate-600 dark:text-slate-300">
                        {r}
                      </span>
                    ))}
                  </div>
